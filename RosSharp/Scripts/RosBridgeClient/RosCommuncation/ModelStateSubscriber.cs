...
{
    public class ModelStateSubscriber : UnitySubscriber<MessageTypes.Gazebo.ModelStates>
    {
        public List<string> LinkNames;
        public List<ModelStateWriter> ModelStateWriters;

        protected override void ReceiveMessage(MessageTypes.Gazebo.ModelStates message)
        {
            int index;
            for (int i = 0; i < message.name.Length; i++)
            {
                ...
                if (index != -1)
                {
                    ...
                    ModelStateWriters[index].Write(pos,rot);
                }
            }
        }
    }
